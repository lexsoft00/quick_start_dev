...
class QuickStartContext extends RawDrupalContext implements SnippetAcceptingContext {
  ...
  */
  ...
  public function __construct(array $parameters) {
    ...
    if (isset($parameters['quick_start_users'])) {
      $this->quick_start_users = $parameters['quick_start_users'];
      foreach ($parameters['quick_start_users'] as $quick_start_username => $quick_start_user) {
        $this->users[$quick_start_username] = $quick_start_user['password'];
      }
    }
    else {
      throw new Exception('behat.yml config files should include "quick_start_users" property.');
    }
  }
  ...
  */
  ...
  /**
   * #drupalcoders : Move the focus to selected rich text editor field.
   *
  ...
  /**
  * #drupalcoders : Select all text in selected field input element.
  *
 ...
 * @Then /^I should see "(?P<text>[^"]*)" value in the "(?P<selector>[^"]*)" input element$/
  ...
  */
  ...
  /**
   * #WIP#drupalcoders: To press a modifier and other key
